...
TEST(costTest, controlSquareGradient_isZero_whenControlsAreAllZero) { 
	const unsigned numberOfPoints = 3;
	const unsigned pointDimension = 3;
	const unsigned controlDimension = 1;
	const auto trajectoryDimension = numberOfPoints * pointDimension;
	...
	const std::vector<double> expected(trajectoryDimension);
	EXPECT_EQ(expected, getControlSquareSumGradient(trajectoryWithZeroControl_RawDouble));
}


TEST(costTest, controlSquareGradientWhenControlsAreThreeTwo) {
	using namespace ranges;
	const unsigned numberOfPoints = 3;
	const unsigned pointDimension = 3;
	const unsigned controlDimension = 1;
...
TEST(costTest, controlSquareGradientWhenControlsAreThreeTwoTwo) { 
	using namespace ranges;
	const unsigned numberOfPoints = 3;
	const unsigned pointDimension = 4;
	const unsigned controlDimension = 2;
	...
	const std::vector<double> expectedPoint = {0, 0, 4, 4};
	const std::vector<double> expected = createTrajectoryWithIdenticalPoints(
																	numberOfPoints,
																	expectedPoint);
	auto getControlSquareSumGradient = GetControlSquareSumGradient(numberOfPoints,
													pointDimension,
													controlDimension);
	EXPECT_EQ(expected, getControlSquareSumGradient(trajectoryWithControlTwoTwo_ptr));
}
